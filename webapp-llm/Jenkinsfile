pipeline {
    ...
    environment {
        DOCKERHUB_REPO = 'bala699/webapp-llm'
        BUILD_NUMBER = 'latest'
        GH_TOKEN = credentials('github-pat')
        REPO_NAME = "webapp-llm"
        REPO_OWNER = "cyse7125-su24-team12"
        DOCKER_BUILDER_NAME = 'webapp-llm-builder'
    ...
    stages {
        stage('Checkout') {
            ...
            userRemoteConfigs: [[url: 'https://github.com/cyse7125-su24-team12/webapp-llm.git', credentialsId: 'git-credentials-id']]
        ])
            }
        }
        stage('Setup Buildx') {
        ...
        stage('Lint Dockerfile') {
            ...
            steps {
                script {
                    echo 'Linting Dockerfile webapp'
                    sh 'hadolint $DOCKER_FILE'
                }
            }
        }
        stage('Build and push the docker image using buildx') {
            ...
            # Check if the builder 'db-webapp-builder' already exists
            ...
            # Build and push Docker image for the webapp
            docker buildx build --platform linux/amd64,linux/arm64 -t ${DOCKERHUB_REPO}:${release_tag} -t ${DOCKERHUB_REPO}:${BUILD_NUMBER} -f ${DOCKER_FILE} --push .
    ...
    post {
        ...
        failure {
            echo 'Docker image/linting push failed! Check the build logs for more details.'
        }
    }
}
